...
<script type="text/javascript"> 
	$(function(){
		$("#btn2").click(function(){
			console.log($("#memberslist").css("display"));
			...
			if($("#memberslist").css("display")=="block"){			
				$.ajax({
					...
					success:function(data){
						var adminlist=$("#adminlist").children().last();
						console.log(adminlist);   
						for(var i=0;i<data.length;i++){							
							var str="<tr>"+"<td>"+data[i].admin_num+"</td>"+"<td>"+data[i].admin_id+"</td>"
							+"<td>"+data[i].admin_nick+"</td>"+"<td>"+data[i].admin_email+"</td>"
							+"<td>"+data[i].admin_regdate+"</td>"+"</tr>";
...
<div id="memberslist">
<h1>회원정보<button class="btn btn-md" id="btn2">관리자정보</button></h1>
	<table class="table table-striped">
		...
		<c:forEach var="user" items="${mblist }">
			<tr>
				...
				<td><a href="mbdelete?user_num=${user.user_num }"><i
...
<h1>관리자정보<button class="btn btn-md" id="btn3">회원정보</button></h1>
	<table class="table table-striped">
		<tr>
			<th>관리자번호</th>
			<th>관리자ID</th>
...
<div class="btn-group">
	<button class="btn btn-lg" data-toggle="modal"
		data-target="#mbgetinfo_modal" id="mbgetinfo">등록</button>
