...
AS
    SELECT MC.*,AA.CNT,M.USER_ID,M.USER_NICKNAME,MSP.MSP_SAVIMG
...
CREATE OR REPLACE VIEW COMMVOTELIST
AS
	SELECT CV.VOTE_NUM,CV.COMM_NUM,CV.USER_NUM,CV.VOTE_TITLE,CV.VOTE_INFO,ROUND(CV.VOTE_END-CV.VOTE_BEGIN) REMAIN,
	MEM.USER_NICKNAME,MEM.USER_EMAIL,
	MSP.MSP_SAVIMG,
    NVL(AA.VOTECNT,0) CNT
	FROM COMM_VOTE CV,MEMBERS MEM,MY_SKIN MS,MY_SKIN_PROFILE MSP,
    (
        SELECT CVV.VOTE_NUM,NVL(COUNT(*),0) VOTECNT
        FROM COMM_VOTE CVV,VOTING_OPTION VO,VOTING_USERLIST VU
        WHERE CVV.VOTE_NUM=VO.VOTE_NUM AND VO.VO_NUM=VU.VO_NUM
        GROUP BY CVV.VOTE_NUM
    )AA
	WHERE CV.USER_NUM=MEM.USER_NUM AND MEM.USER_NUM=MS.USER_NUM 
    AND MS.MS_NUM=MSP.MS_NUM AND MS.MS_USING=1 AND AA.VOTE_NUM(+)=CV.VOTE_NUM;
