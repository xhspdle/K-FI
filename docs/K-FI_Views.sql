CONN system/admin
GRANT CREATE VIEW TO kfi;

CREATE OR REPLACE VIEW MYBOARDLIST
AS
	SELECT MB.MB_NUM,MB.USER_NUM,MB.MB_TITLE,MB.MB_CONTENT,MB.MB_DATE,MB.MB_VIEWS,AA.COMMENT_CNT,BB.LIKE_CNT,MP.MP_NUM,MP.MP_SAVIMG,MV.MV_NUM,MV.MV_SAVVID
	FROM MY_BOARD MB,
	(
...
AS
    SELECT MC.*,AA.CNT,M.USER_ID
    FROM MY_COMMENT MC,MEMBERS M,
    (
        SELECT NVL(COUNT(MCL_NUM),0) CNT,MYC_NUM
        FROM MY_COMMENT_LIKE
        GROUP BY MYC_NUM
    )AA
    WHERE AA.MYC_NUM(+)=MC.MYC_NUM AND M.USER_NUM=MC.USER_NUM
